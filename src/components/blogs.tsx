...
type BlogWithAuthor = Awaited<ReturnType<typeof getAllBlogsInDb>>[number]

export default function Blogs({ blogs } : { blogs : Promise<BlogWithAuthor[]>}) {
    ...
    if (status === 'deleted') {
      setAlertInfo({ status: "success", message: "Blog deleted successfully" })
    } else if (status === 'error') {
      setAlertInfo({ status: "error", message: "Deletion failed" })
    }
    ...
    return (
        <>
            {allBlogs.length === 0 ? (
                <div className="text-xl text-gray-700 h-[80vh] flex items-center">No blogs</div>
            ) : (
                <ul className="grid grid-cols-1 gap-4 max-w-4xl">
                    {allBlogs.map((blog) => (
                        <Link href={`/blogs/${encodeURIComponent(blog.slug)}`} key={blog.id}>
                            <li>
                                <Card className="w-full border border-green-400">
                                    <CardHeader className="flex gap-2 items-center">
                                        <Avatar className="border">
                                            <AvatarImage  src={blog.author.avatar}/>
                                            <AvatarFallback>U</AvatarFallback>
                                        </Avatar>
                                        <CardTitle>{blog.author.name}</CardTitle>
                                    </CardHeader>
                                    <CardContent className="flex gap-1 items-start">
                                        <div>
                                            <h1 className="font-bold text-3xl">
                                                <span className={cn(oxanium.className,"text-wrap")}>{blog.title}</span>
                                            </h1>

...
import { oxanium, spaceMono } from "@/lib/fonts";
import TiptapContentToText from "./tiptap-content-to-text";
import { format } from 'date-fns'

type AuthorWithBlogs = NonNullable<Awaited<ReturnType<typeof getAuthorDetails>>>

type BlogType = AuthorWithBlogs["posts"]


export default function AuthorProfileBlogs({ blogs } : { blogs : BlogType | undefined}) {

    return (
        <div className="mt-5">
            {blogs?.length === 0 ? (
                <div className="text-slate-700 h-[55vh] flex items-center justify-center">No blogs</div>
            ) : (
                <ul className="grid grid-cols-1 lg:grid-cols-2 gap-7 md:p-4">
                    {blogs?.map((blog) => (
                        <Link href={`/blogs/${blog.slug}`} key={blog.id}>
                            <li>
                                <Card className='border border-green-500 h-64 flex flex-col justify-between'>
                                    <CardHeader className="flex gap-2 items-center">
                                        <CardTitle className={cn(oxanium.className,"text-2xl text-wrap line-clamp-2")}>{blog.title}</CardTitle>
                                    </CardHeader>
                                    <CardContent className="flex gap-1 items-start">
                                        <TiptapContentToText content={JSON.stringify(blog.content)} />
                                    </CardContent>
                                    <CardFooter className="flex gap-5 items-center">
                                        <div className={cn(spaceMono.className,"text-sm")}>{format(new Date(blog.createdAt),"MMMM do yyyy")}</div>
